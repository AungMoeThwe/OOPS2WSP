	...
	std::ostream& Book::write(std::ostream& os) const {
		...
		if (conIO(os)) {
			...
			if (m_authorName) {
				...
				os.write(m_authorName, len < SENECA_AUTHOR_WIDTH ? len : SENECA_AUTHOR_WIDTH);
				if (len < SENECA_AUTHOR_WIDTH) {
					os << std::setw(SENECA_AUTHOR_WIDTH - len) << std::setfill(' ') << "";
				}
			}
			else {
				os << std::setw(SENECA_AUTHOR_WIDTH) << std::setfill(' ') << "";
			}
			os << " |";
		}
		else {
